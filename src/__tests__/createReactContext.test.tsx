...
class ThemeToggler extends React.Component<any, { theme: Theme }> {
...
test('can skip consumers with bitmask', () => {
	...
	class Indirection extends React.Component<any> {
		shouldComponentUpdate() {
			return false;
		}
		render() {
	...
	expect(wrapper.contains(<span id="Foo: 1" />)).toBe(true);
	expect(wrapper.contains(<span id="Bar: 1" />)).toBe(true);

	// Update only foo
	wrapper.setProps({ foo: 2, bar: 1 });
	expect(renders.Foo).toBe(2);
	expect(renders.Bar).toBe(1);
	expect(wrapper.contains(<span id="Foo: 2" />)).toBe(true);
	expect(wrapper.contains(<span id="Bar: 1" />)).toBe(true);
	...
	expect(wrapper.contains(<span id="Foo: 2" />)).toBe(true);
	expect(wrapper.contains(<span id="Bar: 2" />)).toBe(true);

	// Update both
	wrapper.setProps({ bar: 3, foo: 3 });
	expect(renders.Foo).toBe(3);
	expect(renders.Bar).toBe(3);
	expect(wrapper.contains(<span id="Foo: 3" />)).toBe(true);
	expect(wrapper.contains(<span id="Bar: 3" />)).toBe(true);
});
